...
describe("agent StartAgentPrompt", function()
  ...
  it("opens prompt in main worktree on work branch", function()
    ...
    assert.same({ { path = "/home/test/.local/state/uc/github/hjdivad/dotfiles", flags = "p" } }, mkdir_calls)
    assert.same({
      "cd " .. vim.fn.fnameescape("/home/test/.local/state/uc/github/hjdivad/dotfiles"),
      "edit " .. vim.fn.fnameescape("/home/test/.local/state/uc/github/hjdivad/dotfiles/uc-987.prompt.md"),
    }, cmd_calls)
  end)
  ...
  it("uses fallback task name on main worktree when branch is not a work branch", function()
    ...
    assert.same({
      "cd " .. vim.fn.fnameescape("/home/test/.local/state/uc/github/hjdivad/dotfiles"),
      "edit " .. vim.fn.fnameescape("/home/test/.local/state/uc/github/hjdivad/dotfiles/task.prompt.md"),
    }, cmd_calls)
  end)
