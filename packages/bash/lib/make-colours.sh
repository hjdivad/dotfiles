...
mkdir -p "$HOME/.bash"
Colors="$HOME/.bash/colors.sh"
echo '' > $Colors

for c in black red green yellow blue cyan white ; do
  echo $c='"'$(color reset $c)'"' >> $Colors
  echo bold_$c='"'$(color $c bold)'"' >> $Colors
  echo underline_$c='"'$(color $c underline)'"' >> $Colors
  echo background_$c='"'$(color $c bg)'"' >> $Colors
done

echo purple='"'$(color reset magenta)'"' >> $Colors
echo bold_purple='"'"$(color magenta bold)"'"' >> $Colors
echo underline_purple='"'"$(color magenta underline)"'"' >> $Colors
echo background_purple='"'"$(color magenta bg)"'"' >> $Colors

echo orange='"'$(color reset red fg bright)'"' >> $Colors
echo bold_orange='"'"$(color red fg bright bold)"'"' >> $Colors
echo underline_orange='"'"$(color red fg bright underline)"'"' >> $Colors
echo background_orange='"'"$(color red bg bright)"'"' >> $Colors

echo normal='"'"$(color reset)"'"' >> $Colors
echo reset_color='"'"$(__make_ansi '' 39)"'"' >> $Colors
